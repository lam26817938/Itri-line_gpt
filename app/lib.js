...
const FORMURL='https://www.itri.org.tw/'
...
const cases={
   ...
   const casetest={
      "備料預測": "國營事業備料預測：將多種物料的使用量資料，透過時序預測模組建立AI模型與成效評估",
      "交貨量預測": "國營事業物料需求量預測：根據倉庫每種物料的需求量，透過時序預測模組建立AI模型與成效評估",
      "AOI瑕疵分類": "製造業AOI瑕疵分類：將AOI影像資料，透過影像標註與圖片辨識模組建立AI模型，分類AOI影像瑕疵",
      "銷量預測": "零售業銷量預測：將商品歷史日銷售資料，透過時序預測模組建立AI模型與成效評估",
      "推薦系統": "顧客喜好推薦：利用自然語言處理和深度學習技術，分析顧客購買歷史，推薦相關產品",
      "售價訂定": "定價策略智慧化：利用FAST AI分析消費者對不同價格點的反應，從而制定最適合的訂價策略",
      "員工離職預測": "分析員工未來的離職風險：將歷史員工離職資料，透過時序預測模組建立AI模型與成效評估，分析員工離職風險",
      "員工表現評估": "員工工作表現考核：利用機器學習模型，對員工的表現數據進行分析",
      "景點人數預測": "景點人數預測：結合電信人群資料和外部資料如COVID-19、Google Trend等資料",
      "載客熱點預測": "載客熱點預測：結合計程車GPS紀錄、歷史乘車資料、地點資料和外部資訊如結合日曆、天氣等資訊",
      "馬拉松運動博覽會參訪動線類別預測": "分析活動人潮路線預測群眾喜好：利用參與者的手機移動路徑資料，透過時序預測模組",
      "智慧交通": "分析交通熱區與交通管理：透過圖像識別和機器學習技術，進行交通監控和智慧交通管理",
      "房價預測": "控制房價漲幅：利用不動產交易歷史資料，透過時序預測模組建立AI模型，預測未來房價走勢",
      "循環材料辨識": "循環材料辨識：將卡車車斗內可辨識循環材料的影像資料，透過影像辨識模組建立AI模型",
      "智慧建築": "建築物能耗預測：利用建築物的歷史能耗數據和其他數據，透過時序預測和影像辨識模組建立AI模型",
      "疾病風險預測": "糖尿病患者心血管疾病1年內的風險預測：利用眼底影像和患者資料，透過時序預測和影像辨識模組建立模型",
      "糖尿病視網膜病變影像判讀": "糖尿病視網膜病變病徵/組織結構位置偵測：利用眼底影像和標註資料，透過影像辨識模組建立AI模型",
      "腦腫瘤分割": "腦腫瘤位置偵測與腫瘤分割：使用醫大腦 MRI 影像資料，透過影像辨識模組建立AI模型，快速偵測腫瘤位置",
      "農產品瑕疵檢測": "農產品品質分類：利用已標記檸檬品質好與不好的影像資料，透過影像辨識模組建立AI模型",
      "台灣牧場乳量預測": "預測台灣不同地區牧場生產的乳量：利用乳業協會的乳牛群性能改良資料，透過時序預測模組建立AI模型"
      }
...
export {cases, industry, title, casetest, URLPREFIX, GPTFOOD, QQQQ, VIDEOIMG, FORMURL};